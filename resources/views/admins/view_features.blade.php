...
@section('content')
    ...
    <!-- Page Content -->
    <!-- add Feature modal -->
      <div class="modal fade" style="margin-left: -250px; margin-top: 20px;" id="FeatureModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content" style="width:200%;">
          ...
          <div class="modal-body">
            ...
            <form method="post" role="form" class="form-horizontal" id="feature_form">
              @csrf
              <div class="form-group row add">
                <label for="feature_name" class="control-label col-sm-3" style="font-weight: 600;">Name :</label>
                <div class="col-sm-9">
                  <input type="text" name="feature_name" id="feature_name" style="border-radius: 5px;" class="form-control" placeholder="Enter Feature Name" autocomplete="off" autofocus required/>
                </div>
              </div>
              <div class="form-group row add">
                <label for="parent_feature_id" class="control-label col-sm-3" style="font-weight: 600;">Parent Feature :</label>
                <div class="col-sm-9">
                  <select class="form-control" id="parent_feature_id" style="border-radius: 5px;" name="parent_feature_id">
                    <option value="">Select Parent Feature</option>
                    <?php if(isset($feats) && count($feats) > 0){ ?>
                      @foreach($feats as $feat)
                      <option value="{{ $feat->id }}">{{ $feat->feature_name }}</option>
                      @endforeach
                    <?php } ?>
                  </select>
                </div>
              ...
              <div class="form-group row add">
    						<label for="feature_icon" class="control-label col-sm-3" style="font-weight: 600;">Upload Feature Icon :</label>
    						<div class="col-sm-9">
    							<input type="file" class="form-control image" id="feature_icon" name="feature_icon" style="border-radius: 5px;">
    						</div>
    					</div>
    ...
    <!-- add Feature modal -->
    <!-- edit Feature modal -->
      <div class="modal fade" style="margin-left: -250px; margin-top: 20px;" id="EditFeatureModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      	<div class="modal-dialog" role="document">
      		<div class="modal-content" style="width:200%;">
      			<div class="modal-header">
      			...
      			<div class="modal-body">
      				...
      				<form method="post" role="form" class="form-horizontal" id="edit_feature_form" enctype="multipart/form-data">
      					@csrf
      					<div class="form-group row add">
      					...
      					<div class="form-group row add">
      						<label for="edit_feature_name" class="control-label col-sm-3" style="font-weight: 500;">Name :</label>
      						<div class="col-sm-9">
      							<input type="text" id="edit_feature_name" name="edit_feature_name" style="border-radius: 5px;" class="form-control" autofocus required/>
      						</div>
      					</div>
                <div class="form-group row add">
                  <label for="edit_parent_feature_id" class="control-label col-sm-3" style="font-weight: 600;">Parent Feature :</label>
                  <div class="col-sm-9">
                    <select class="form-control" id="edit_parent_feature_id" style="border-radius: 5px;" name="edit_parent_feature_id">
                      ...
                      <?php } ?>
      					...
      					<div class="form-group row add">
      						<label for="edit_feature_icon" class="control-label col-sm-3" style="font-weight: 600;">Feature Icon :</label>
      						<div class="col-sm-6">
      							<input type="file" class="form-control image" id="edit_feature_icon" name="edit_feature_icon" autocomplete="off" style="border-radius: 5px;">
      						</div>
      ...
      <div class="modal fade" style="margin-left: -250px; margin-top: 20px;" id="DeleteFeatureModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      	<div class="modal-dialog" role="document">
      		<div class="modal-content" style="width:200%;">
      			<div class="modal-body">
    ...
    <div id="page-content-wrapper">
        ...
        <div class="container-fluid py-3" id="features">
          <!-- table-->
          <div class="card">
...
<script type="text/javascript">
  ...
  $('#feature_form').on('submit', function(event){
      ...
      success:function(data){
        ...
        if(data.errors)
        {
          ...
          var date = moment(data.created_at).format("D MMM YYYY");
					$('tbody').prepend("<tr class='Feature"+data.id+"'>"+
					"<td>" + data.id + "</td>"+
					"<td>" + data.parent_feature_name + "</td>"+
					"<td>" + data.feature_name + "</td>"+
					"<td>" + '<img src={{ asset("/storage") }}/'+data.feature_icon+' width="50px" height="50px">'+ "</td>"+
					"<td>" + date + "</td>"+
					"<td class='px-2 text-nowrap'><a href='#' class='edit_modal btn btn-sm btn-save' data-id='"+data.id+"' data-feature_name='"+data.feature_name+"' data-feature_icon='"+data.feature_icon+"' data-parent_feature_id='"+data.parent_feature_id+"' data-toggle='modal' data-target='#EditFeatureModal' data-whatever='@mdo'>"+
					"<i class='fa fa-pencil'></i> Edit</a> "+
					"<a href='#' class='delete_modal btn btn-sm btn-danger' data-id='"+data.id+"' data-feature_name='"+data.feature_name+"' data-feature_icon='"+data.feature_icon+"' data-toggle='modal' data-target='#DeleteFeatureModal' data-whatever='@mdo'>"+
					...
					$('#append_errors').hide();
					$('#append_success').show();
					$('#append_success ul').append("<li>Feature Created Successfully.</li>");
          $('#FeatureModal').find('#feature_form')[0].reset();
					setTimeout(function(){ $('#append_success').hide(); },1000);
					setTimeout(function(){ $('#FeatureModal').modal('hide'); },2000);
	...
	$(document).on('click', '.edit_modal', function(){
		...
		$('#edit_feature_name').val($(this).data('feature_name'));
    $('#show_image').html('<img src={{ asset("/storage") }}/'+$(this).data('feature_icon')+' width="155px" height="150px">');
    $("#edit_parent_feature_id option[value='"+$(this).data('parent_feature_id')+"']").prop('selected', true);
		$('#edit_append_errors').hide();
		$('#edit_append_success').hide();
	});

	$('#edit_feature_form').on('submit', function(event){
		event.preventDefault();
    $.ajax({
      url:"{{ url('update_feature') }}",
      method:"POST",
			data:new FormData(this),
      dataType:"JSON",
			contentType:false,
        ...
        if(data.errors)
        {
          ...
          var date = moment(data.created_at).format("D MMM YYYY");
					$('.Feature' + data.id).replaceWith(" "+
					"<tr class='Feature"+data.id+"'>"+
					"<td>" + data.id + "</td>"+
					"<td>" + data.parent_feature_name + "</td>"+
					"<td>" + data.feature_name + "</td>"+
					"<td>" + '<img src={{ asset("/storage") }}/'+data.feature_icon+' width="50px" height="50px">'+ "</td>"+
					...
					$('#edit_append_success ul').append("<li>Feature Updated Successfully.</li>");
          setTimeout(function(){ $('#edit_append_success').hide(); },1000);
					setTimeout(function(){ $('#EditFeatureModal').modal('hide'); },2000);
					setTimeout(function(){ $('body').removeClass('modal-open'); },2000);
					setTimeout(function(){ $('.modal-backdrop').remove(); },2000);
        }
      },
	...
	$(document).on('click', '.delete_modal', function(){
		$('.title').html($(this).data('feature_name'));
  ...
  $('.delete').on('click',function(event){
    ...
    $.ajax({
        ...
        success:function(data){
          ...
          $('.Feature' + $('.id').text()).remove();
					setTimeout(function(){ $('#DeleteFeatureModal').modal('hide'); },3000);
